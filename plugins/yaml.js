(function (f) {
	...
	var rr = (t, e, n) => ((n = t != null ? Ai(Mi(t)) : {}), nr(e || !t || !t.__esModule ? yt(n, "default", { value: t, enumerable: !0 }) : n, t)),
					...
					let y = !1,
						...
						g = this.items.reduce((C, L, M) => {
							...
							L &&
								...
								m && ((!y && L.spaceBefore) || L.commentBefore || L.comment || (L.key && (L.key.commentBefore || L.key.comment)) || (L.value && (L.value.commentBefore || L.value.comment))) && (h = !0)),
								(y = !1);
							let _ = f(
								L,
								e,
		...
		var Ut = (t, e) => {
			...
			we = class t extends V {
				...
				toJSON(e, n) {
					...
					if (s >= 0 && ((i.count += 1), i.aliasCount === 0 && (i.aliasCount = Ut(this.source, r)), i.count * i.aliasCount > s)) {
...
`
		...
		function jt(t, e, n, { indentAtStart: r, lineWidth: s = 80, minContentWidth: i = 20, onFold: o, onOverflow: a }) {
			if (!s || s < 0) return t;
...
${e}${t.slice(A + 1, _)}`));
		...
		var qn = ({ indentAtStart: t }) => (t ? Object.assign({ indentAtStart: t }, be.fold) : be.fold),
			Qt = (t) => /^(%|---|\.\.\.)/m.test(t);
		function Do(t, e, n) {
			if (!e || e < 0) return !1;
			let r = e - n,
...
${a}`
		...
		function Qo(t, e) {
			...
			let { comments: n, items: r } = e.type === p.Type.FLOW_MAP ? Xo(t, e) : Ho(t, e),
				s = new mt();
			(s.items = r), Ws(s, n);
			let i = !1;
...
`
				...
				if (typeof c.char == "string") {
					...
					c.type === p.Type.BLANK_LINE
						...
						: c.type === p.Type.COMMENT
							...
							: s === void 0
								? (o === "," && t.errors.push(new p.YAMLSemanticError(c, "Separator , missing in flow map")), (s = me(t, c)))
								: (o !== "," && t.errors.push(new p.YAMLSemanticError(c, "Indicator : missing in flow map entry")), r.push(new T(s, me(t, c))), (s = void 0), (i = !1));
			}
		...
		function zo(t, e) {
			...
			let { comments: n, items: r } = e.type === p.Type.FLOW_SEQ ? ea(t, e) : Zo(t, e),
				s = new pe();
...
${i.comment}`
		...
		var ca = {
			...
			Gs = ({ value: t }) => {
				...
				let n = [t % 60];
				return (
					t < 60 ? n.unshift(0) : ((t = Math.round((t - n[0]) / 60)), n.unshift(t % 60), t >= 60 && ((t = Math.round((t - n[0]) / 60)), n.unshift(t))),
					e +
						n
	...
	var Hn = ee((ci) => {
		...
		var Xs = ({ value: t }) => (t ? E.boolOptions.trueStr : E.boolOptions.falseStr),
		...
		var Xn = class t {
			...
			en = (t, e) => {
				if (t && typeof t == "object") {
					let { tag: n } = t;
					t instanceof S.Collection ? (n && (e[n] = !0), t.items.forEach((r) => en(r, e))) : t instanceof S.Pair ? (en(t.key, e), en(t.value, e)) : t instanceof S.Scalar && n && (e[n] = !0);
				}
				return e;
			},
		...
		var tn = class t {
			...
			toString() {
				...
				this.tagPrefixes.forEach(({ handle: c, prefix: l }) => {
					...
					this.commentBefore && ((r || !this.directivesEndMarker) && n.unshift(""), n.unshift(this.commentBefore.replace(/^/gm, "#")));
				let i = {
						anchors: Object.create(null),
						doc: this,
						indent: "",
				...
				return (
					this.comment && ((!o || a) && n[n.length - 1] !== "" && n.push(""), n.push(this.comment.replace(/^/gm, "#"))),
					n.join(`
`) +
						`
`
...
`,
		...
		return n.proseWrap === "preserve"
			...
			: r
					...
					.reduce((s, i, o) => (o !== 0 && r[o - 1].length > 0 && i.length > 0 && !(t === "quoteDouble" && x(!1, x(!1, s, -1), -1).endsWith("\\")) ? [...s.slice(0, -1), [...x(!1, s, -1), ...i]] : [...s, i]), [])
...
`,
		...
		return a(
			o
				...
				.reduce((c, l, f) => (f !== 0 && o[f - 1].length > 0 && l.length > 0 && !/^\s/u.test(l[0]) && !/^\s|\s$/u.test(x(!1, c, -1)) ? [...c.slice(0, -1), [...x(!1, c, -1), ...l]] : [...c, l]), [])
				.map((c) => c.reduce((l, f) => (l.length > 0 && /\s$/u.test(x(!1, l, -1)) ? [...l.slice(0, -1), x(!1, l, -1) + " " + f] : [...l, f]), []))
	...
	function Ct(t, e) {
		let { node: n, root: r } = t,
			s;
		return hn.has(r) ? (s = hn.get(r)) : ((s = new Set()), hn.set(r, s)), !s.has(n.position.end.line) && (s.add(n.position.end.line), br(n, e) && !gn(t.parent)) ? Ot : "";
	}
	function gn(t) {
	...
	function Qi(t, e, n) {
		...
		let a = Ar(r, { parentIndent: s, isLastDescendant: i, options: n }),
			c = [];
		for (let [l, f] of a.entries()) l === 0 && c.push(N), c.push(bt(v(ne, f))), l !== a.length - 1 ? c.push(f.length === 0 ? N : ir(rt)) : r.chomping === "keep" && i && c.push(on(f.length === 0 ? N : rt));
	...
	function Ji(t, e, n) {
		return t.map(({ isLast: r, node: s, next: i }) => [e(), r ? "" : [",", ne, s.position.start.line !== i.position.start.line ? Ct(t, n.originalText) : ""]], "children");
	}
	function Gi(t, e, n) {
		var C;
...
`);
	...
	function eo(t, e, n) {
		...
		let a = "";
		return (
			G(r, ["mapping", "sequence", "comment", "directive", "mappingItem", "sequenceItem"]) && !At(t) && (a = Ct(t, e.originalText)),
			(i || o) && (G(r, ["sequence", "mapping"]) && !ie(r) ? s.push(N) : s.push(" ")),
			ie(r) && s.push([r.middleComments.length === 1 ? "" : N, v(N, t.map(n, "middleComments")), N]),
			Nr(t) ? s.push(cr(e.originalText.slice(r.position.start.offset, r.position.end.offset).trimEnd())) : s.push(ke(to(t, e, n))),
			H(r) && !G(r, ["document", "documentHead"]) && s.push(ar([r.type === "mappingValue" && !r.content ? "" : " ", t.parent.type === "mappingKey" && t.getParentNode(2).type === "mapping" && ot(r) ? "" : Nt, n("trailingComment")])),
			gn(r) &&
				s.push(
					I(r.type === "sequenceItem" ? 2 : 0, [
						...
						v(
							N,
							t.map(({ node: c }) => [lr(e.originalText, ve(c)) ? N : "", n()], "endComments"),
						),
					]),
				),
			s.push(a),
	...
	function to(t, e, n) {
		...
		switch (r.type) {
			...
			case "quoteSingle": {
				...
				if (o.includes(i)) return [s, at(r.type, r.type === "quoteDouble" ? Et(!1, Et(!1, o, String.raw`\"`, i), "'", s.repeat(2)) : o, e), s];
				if (o.includes(s)) return [i, at(r.type, r.type === "quoteSingle" ? Et(!1, o, "''", s) : o, e), i];
...
`,
	...
	function ts(t, e, n) {
		let r = t.cstNode,
			{ comments: s, endComments: i, documentTrailingComment: o, documentHeadTrailingComment: a } = lo(r, e, n),
	...
	function rs(t, e) {
		...
		return (
			e.comments.push(...s, ...i),
			{
				createDocumentHeadWithTrailingComment: (l) => (l && e.comments.push(l), ns(o, r, i, l)),
	...
	function ss(t, e) {
		let { createDocumentHeadWithTrailingComment: n, documentHeadEndMarkerPoint: r } = rs(t, e),
			{ documentBody: s, documentEndPoint: i, documentTrailingComment: o, documentHeadTrailingComment: a } = ts(t, e, r),
			c = n(a);
